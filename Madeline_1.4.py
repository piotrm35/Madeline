...
class Madeline(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.DECYZJE_FOLDER_PATH = 'xxxxxx'
        ...
        self.Usun_skladajacy_pushButton.clicked.connect(self.Usun_skladajacy_pushButton_clicked)
        ...
        self.Usun_inwestor_pushButton.clicked.connect(self.Usun_inwestor_pushButton_clicked)
        ...
        self.Usun_przedmiot_pushButton.clicked.connect(self.Usun_przedmiot_pushButton_clicked)
    ...
    def closeEvent(self, event):        # overriding the method
        ...
        self.Usun_skladajacy_pushButton.clicked.disconnect(self.Usun_skladajacy_pushButton_clicked)
        ...
        self.Usun_inwestor_pushButton.clicked.disconnect(self.Usun_inwestor_pushButton_clicked)
        ...
        self.Usun_przedmiot_pushButton.clicked.disconnect(self.Usun_przedmiot_pushButton_clicked)
    ...
    def Skladajacy_textEdit_textChanged(self):
        if not self.prompt_used_flag:
            ...
            if len(tx) > 0:
                ...
                self.Usun_skladajacy_pushButton.setEnabled(True)
                self.Prompt_pushButton.setEnabled(True)
            else:
                self.Zapisz_skladajacy_pushButton.setEnabled(False)
                self.Usun_skladajacy_pushButton.setEnabled(False)
    ...
    def Usun_skladajacy_pushButton_clicked(self):
        tx = self.Skladajacy_textEdit.toPlainText().strip()
        tx = self.clear_text(tx)
        if len(tx) > 0:
            if self.is_present_in_adresaci_TAB(tx):
                DELETE_SQL = "DELETE FROM adresaci WHERE adres LIKE '" + tx + "';"
                print("Usun_skladajacy_pushButton_clicked, DELETE_SQL = " + str(DELETE_SQL))
                result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                if result == 'ERROR':
                    self.DOC_dB_connection_PostgreSQL.Restart_DB_connection()
                    result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                print("Usun_skladajacy_pushButton_clicked, result = " + str(result))
            else:
                QtWidgets.QMessageBox.information(self, SCRIPT_TITLE, 'TEKSTU BRAK W BAZIE DANYCH')
        self.Usun_skladajacy_pushButton.setEnabled(False)
    ...
    def Dzialajacy_checkBox_stateChanged(self):
        ...
        self.Usun_inwestor_pushButton.setEnabled(self.Dzialajacy_checkBox.isChecked())
    ...
    def Inwestor_textEdit_textChanged(self):
        if not self.prompt_used_flag:
            ...
            if len(tx) > 0:
                ...
                self.Usun_inwestor_pushButton.setEnabled(True)
                self.Prompt_pushButton.setEnabled(True)
            else:
                self.Zapisz_inwestor_pushButton.setEnabled(False)
                self.Usun_inwestor_pushButton.setEnabled(False)
                self.Prompt_pushButton.setEnabled(False)
        else:
            self.Zapisz_inwestor_pushButton.setEnabled(False)
            self.Usun_inwestor_pushButton.setEnabled(False)
    ...
    def Usun_inwestor_pushButton_clicked(self):
        tx = self.Inwestor_textEdit.toPlainText().strip()
        tx = self.clear_text(tx)
        if len(tx) > 0:
            if self.is_present_in_adresaci_TAB(tx):
                DELETE_SQL = "DELETE FROM adresaci WHERE adres LIKE '" + tx + "';"
                print("Usun_inwestor_pushButton_clicked, DELETE_SQL = " + str(DELETE_SQL))
                result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                if result == 'ERROR':
                    self.DOC_dB_connection_PostgreSQL.Restart_DB_connection()
                    result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                print("Usun_inwestor_pushButton_clicked, result = " + str(result))
            else:
                QtWidgets.QMessageBox.information(self, SCRIPT_TITLE, 'TEKSTU BRAK W BAZIE DANYCH')
        self.Usun_inwestor_pushButton.setEnabled(False)
    ...
    def Analizuj_pushButton_clicked(self):
        ...
        if parcel_list:
            ...
            if len(parcel_list) != len(self.parcel_select_sql_result):
                tmp_dzialki_list = parcel_list.copy()
                for res in self.parcel_select_sql_result:
                    del(tmp_dzialki_list[tmp_dzialki_list.index(res[0])])
                print('Analizuj_pushButton_clicked PROBLEM: len(parcel_list) != len(self.parcel_select_sql_result), BRAK DANYCH DLA: ' + str(tmp_dzialki_list))
                QtWidgets.QMessageBox.information(self, SCRIPT_TITLE, 'len(parcel_list) != len(self.parcel_select_sql_result), BRAK DANYCH DLA: ' + str(tmp_dzialki_list))
            print('\n-----------------------------------------------------------------------------------')
    ...
    def ustaw_dzialki_ulice_i_szablon(self, parcel_select_sql_result, automat_map, idx):
        ...
        if automat_map_keys[idx].startswith('Uzgodnienie i opinia'):
            ...
            tmp_dzialki_list += automat_map[automat_map_keys[idx]][0]
            tmp_dzialki_list += automat_map[automat_map_keys[idx]][1]
        else:
            self.Numery_dzialek_textEdit.setText(', '.join(automat_map[automat_map_keys[idx]]))
            tmp_dzialki_list += automat_map[automat_map_keys[idx]]
        print('tmp_dzialki_list = ' + str(tmp_dzialki_list))
        for p in tmp_dzialki_list:
    ...
    def Przedmiot_textEdit_textChanged(self):
        if not self.prompt_used_flag:
            ...
            if len(tx) > 0:
                ...
                self.Usun_przedmiot_pushButton.setEnabled(True)
                self.Prompt_pushButton.setEnabled(True)
            else:
                self.Zapisz_przedmiot_pushButton.setEnabled(False)
                self.Usun_przedmiot_pushButton.setEnabled(False)
                self.Prompt_pushButton.setEnabled(False)
        else:
            self.Zapisz_przedmiot_pushButton.setEnabled(False)
            self.Usun_przedmiot_pushButton.setEnabled(False)
    ...
    def Usun_przedmiot_pushButton_clicked(self):
        tx = self.Przedmiot_textEdit.toPlainText().strip()
        tx = tx.replace('\n', ' ')
        tx = tx.replace(' ,', ', ')
        tx = self.replace_loop(tx, '  ', ' ')
        tx = tx.strip()
        if len(tx) > 0:
            if self.is_present_in_przedmioty_TAB(tx):
                DELETE_SQL = "DELETE FROM przedmioty WHERE przedmiot LIKE '" + tx + "';"
                print("Usun_przedmiot_pushButton_clicked, DELETE_SQL = " + str(DELETE_SQL))
                result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                if result == 'ERROR':
                    self.DOC_dB_connection_PostgreSQL.Restart_DB_connection()
                    result = self.DOC_dB_connection_PostgreSQL.Send_SQL_to_DB(DELETE_SQL)
                print("Usun_przedmiot_pushButton_clicked, result = " + str(result))
            else:
                QtWidgets.QMessageBox.information(self, SCRIPT_TITLE, 'TEKSTU BRAK W BAZIE DANYCH')
        self.Usun_przedmiot_pushButton.setEnabled(False)
        ...
        if template_file_name not in self.DOC_TEMPLATES_NO_SENDER_LIST:
            ...
            if self.Dzialajacy_checkBox.isChecked():
                ...
                if not adres_inwestora:
                    ...
                    return False
                adres_strony = adres_skladajacego + '\ndziałający(a) w imieniu i na rzecz\n' + adres_inwestora
            else:
                adres_strony = adres_skladajacego
    ...
    def fill_gridLayout(self, sql):
        ...
        for record in result:
            self.gridLayout.addWidget(self.get_QLabel(self.restore_text(str(record[0]))), row, 0)
            row += 1
        _new_rect = QtCore.QRect(0, 0, self.My_QLabel_WIDTH, row * (self.My_QLabel_HEIGHT + 2 * self.My_QLabel_SEP + self.BOTTOM_SPACE))
    ...
    def get_QLabel(self, text):
        _tmp_label = My_QLabel(self, str(text))
